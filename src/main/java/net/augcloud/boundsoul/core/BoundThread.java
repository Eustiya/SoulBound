...
public class BoundThread {
    ...
    public void startThread() {
        ...
        this.updataInvThread.start();
        BoundSoul.plugin.getLogger().info("线程运行中");
    }
    ...
    private void checkThread() {
                ...
                while (BoundThread.this.threadRunning && this.running) {
                    try {
                        if (!BoundThread.this.updataInvThread.isAlive()) {
                            BoundThread.this.updataInvThread.stop();
                            BoundThread.this.startThread();
                            BoundSoul.plugin.getLogger().info("发现检测线程不运行，自动激活了它");
                        }
                        BoundSoul.illegalPlayer.ruin();
                        Thread.sleep(20000L);
                    } catch (InterruptedException e) {
                        BoundSoul.plugin.getLogger().info("检测线程崩溃");
                        this.running = false;
        ...
        this.checkThread.start();
        BoundSoul.plugin.getLogger().info("检测线程运行，此线程会每10秒确认一次主线程存活，如果停止会自动重启");
    }
}